            ...
            if($num){
                ...
                if($resultado){
                    ...
                    while($r = mysqli_fetch_array($resultado)){
                        ...
                        $condicional_tickets .= ", ".$r['fk_tienda'];
                    }
                }
                ...
                if($resultado){
                    ...
                    $detallesTicket = $temp;
                    $dataResponse['datos_generales'] = array('costo_total' => $costo_total, 'venta_total' => $venta_total, 'abono_total' =>$abono_total, 'ganancia' => $ganancia);
                ...
                if($resultado){
                    ...
                    while($r = mysqli_fetch_array($resultado)){
                        $temp[$c] = array('fk_ticket' => $r['fk_ticket'], 'abono' => $r['abono']);
                        $abonos_otrosPlazos += $r['abono'];
                        $condicional_tickets .= ", ".$r['fk_tienda']; //posible error aqui
                    }
                }
                ...
                $c= 0;
                $temp = array();
                $condicional = "id_ticket NOT IN (".$condicional_tickets.") AND fk_tienda = ".$id_tienda." AND fecha BETWEEN '".$fInicio."' AND '".$fFinal."'";
                $sqlString = "SELECT * FROM tickets WHERE ".$condicional;
                $resultado = mysqli_query($conn,$sqlString);
                $num = mysqli_num_rows($resultado);
                //posible error, prueba sin necesidad de un if
